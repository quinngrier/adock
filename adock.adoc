...
:x_quinngrier_adock_latest: pass:n[**docker.io/quinngrier/adock**]
...
:x_opt_u_name_arg: pass:n[``-u``]
:x_podman: pass:n[``podman``]
:x_quinngrier_adock_latest: pass:n[link:https://hub.docker.com/r/quinngrier/adock[``docker.io/quinngrier/adock``]]
...
If neither command exists, the default value is {x_docker}, and any
attempt to run it will fail.

{x_var_DOCKER_FLAGS}::
Extra arguments to pass to {x_docker} before any subcommand.
The value will be evaluated by the shell.
If the variable is unset, the default value is empty.
